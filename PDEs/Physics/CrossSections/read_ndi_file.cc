...
namespace PDEs
{
  namespace Physics
  {
    void CrossSections::read_ndi_file(const std::string file_name,
                                      ...
                                      const bool verbose)
    {
      ...
      auto read_transfer_matrix =
             ...
             size_t& line_number)
          {
            ...
            while (true)
            {
               ...
               * contiguously written and is of a different format than the
               * normal ".xs" file in that the innermost index if the "to"
               * group instead of the "from" group. For this reason, elements
               * are added down columns instead of across rows. When the end
               * of the column is reached, the index counters are moved to the
               * top of the subsequent column. */
              if (vals.size() > 1)
               ...
               * scattering moment. When this occurs, ensure innermost index
               * is at its maximum value and that the moment index is one
               * less than the designated moment index from the file. This
               * moves the counters to  point to the start of the next
               * scattering moment data. */
              else
              {
      ...
      this->compute_scattering_from_transfers();
      this->reconcile_cross_sections();
      this->reconcile_fission_properties();
      this->compute_macroscopic_cross_sections();
