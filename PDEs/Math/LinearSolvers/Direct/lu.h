...
namespace Math::LinearSolver
{
  ...
  public:
     ...
     * \f$ \boldsymbol{U} \f$ is an upper triangular matrix. The factorization
     * is performed in place rather than creating an additional Matrix object.
     *
     * The algorithm used to do perform this factorization is an extension of
     * the formation of a row-echelon form matrix in that the upper triangular
     * matrix is identical to the row-echelon form. The lower triangular matrix
     * then contains the row operations used to form upper triangular system.
     */
     ...
     * \boldsymbol{L} \boldsymbol{U} \vec{x} = \vec{b} \f$. The solution \f$
     * \vec{x} \f$ is obtained in a two step process. First, define \f$ \vec{y}
     * = \boldsymbol{U} \vec{x} \f$ and plug this in to obtain \f$
     * \boldsymbol{L} \vec{y} = \vec{b} \f$. The vector \f$ \vec{y} \f$ can be
     * obtained using forward substitution after reordering the right-hand side
     * vector \f$ \vec{b} \f$ according to the pivot mapping vector. Next,
     * the solution \f$ \vec{x} \f$ is computed using the previous definition
